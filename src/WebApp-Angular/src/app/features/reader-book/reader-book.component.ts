...
import {Book, Chapter} from '../book/models/book.model';
...
export class ReaderBookComponent implements OnInit, OnDestroy {
  ...
  book!: Book;
  isLoadContent = true;
  chapterSlug: string | undefined = undefined;
  ...
  ngOnInit(): void {
    ...
    if(history.state.book){
      ...
      const bookSlug = decodeURIComponent(this.activeRouter.snapshot.params['bookSlug'] || '');
  ...
  loadChapterContent(chapter: Chapter) {
    ...
    this.readerService.getChapterContent(this.book.slug, chapter.slug)
      .subscribe({
        next: (result) => {
          this.isLoadContent = false;
