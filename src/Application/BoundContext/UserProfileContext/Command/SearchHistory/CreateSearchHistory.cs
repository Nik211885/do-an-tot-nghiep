...
public record CreateSearchHistoryCommand(Guid UserId, string SentSearch)
    ...
    : ICommandHandler<CreateSearchHistoryCommand, SearchHistoryViewModel>
{
    ...
    {
        var searchHistory = Core.BoundContext.UserProfileContext.SearchHistoryAggregate.SearchHistory.Create(request.UserId, request.SentSearch);
